            ...
            if ((!License) && (Program.UserData.ContainsKey(UserID)))
            {
                ...
                if (UserData.Obfuscations >= 3)
                {
                    ...
                    else
                    {
                        ...
                        Embed.WithDescription("You have already used the allowed 3 daily obfuscations without owning a premium license. Please upgrade to receive unlimited access to the obfuscator, along with many other restricted features: https://www.psu.dev/upgrade");

                        await ReplyAsync($"{User.Mention}, Thank you for using PSU Obfuscation!", (false), Embed.Build());
